...
~~~sql
事务A：SELECT * FROM table WHERE id BETWEEN 1 AND 5 FOR UPDATE;
事务B：INSERT INTO table (id) VALUES (6);
事务A再次执行范围查询：SELECT * FROM table WHERE id BETWEEN 1 AND 6 FOR UPDATE; 可能会看到新的记录。
~~~
...
​	因为@Transaction的基于AOP实现， AOP cglib的工作原理决定了公共方法才能被代理。
...
​	这是因为Spring AOP工作原理决定的。因为Spring AOP使用动态代理来实现事务的管理，他会在运行的时候为带有@Transaction注解的方法生成代理对象，并在方法调用的前后应用事物逻辑。如果该方法被其他类调用我们的代理对象就会拦截方法调用并处理事物。但是在一个类中的其他方法内部调用(的时候我们代理对象就无法拦截到这个内部调用，因此事物也就失效了。	

当this调用的时候也会失效，因为this指向的不是代理类本身，除非是代理类本身才会生效。
